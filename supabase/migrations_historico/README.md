...
## Estado histórico y estado vivo

Este documento describe **el estado histórico** en el momento de la reconciliación
(2026-08-31): qué había, qué se movió y por qué. Esas cifras están congeladas a propósito y
no se actualizan.

El **estado vivo** del proyecto —qué versiones hay registradas en producción, cuántas
migraciones quedan pendientes y cuál es el techo remoto actual— **no se declara en este
directorio**, porque cualquier cifra escrita aquí quedaría obsoleta en cuanto se añada la
siguiente migración. Para conocerlo hay una única fuente:

```
supabase migration list
```

Lo permanente son las reglas de la sección *"La regla que no hay que romper"* y el campo
`invariante` de `manifest.json`, que están redactadas sin cifras vivas.

## Qué pasó (estado histórico, 2026-08-31)
...
3. Se **movieron a este directorio los 47 ficheros** locales que no tenían fila en producción y
   cuya versión quedaba por debajo del techo remoto de entonces, `20260825094545`.

Con eso el historial quedó reconciliado: a partir de ese punto toda versión registrada en
producción tiene su fichero en `supabase/migrations/`, y el CLI dejó de abortar por los dos
motivos de arriba.

Deliberadamente **no se anota aquí cuántas migraciones hay aplicadas ni cuántas pendientes**.
Ese recuento cambia con cada migración nueva y su fuente es `supabase migration list`.

## La regla que no hay que romper

**No devuelvas ningún fichero de este directorio a `supabase/migrations/`.**

Todas las versiones de aquí son anteriores al techo remoto que había al archivarlas
(`20260825094545`, registrado en `manifest.json` como `techo_remoto_al_archivar`), y por tanto
también a cualquier techo posterior. En cuanto una vuelva, el CLI volverá a abortar con el
error de *"local migration files to be inserted before the last migration"*, y el repositorio
quedará otra vez irreconciliable. La regla no caduca: no depende del techo vigente hoy.
...
Observación hecha durante la reconciliación (estado histórico, 2026-08-31), no un recuento
vigente. Al comparar los 34 ficheros activos de entonces contra
`schema_migrations.statements`, 22 daban `md5`
