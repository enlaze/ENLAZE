...
test("portal token lifecycle issues, rotates and revokes only for the owner",
  ...
  const activeFor = async (project) => Number((await db.query(
    `select count(*) from public.portal_tokens
     where project_id=$1 and is_active and revoked_at is null`, [project])).rows[0].count);
  ...
  await t.test("un proyecto ajeno, inexistente o borrado responden lo mismo", async () => {
    ...
    assert.equal(await activeFor(FOREIGN_PROJECT), 0);
  });
  ...
  await t.test("cada forma inválida de permisos se rechaza", async () => {
    ...
    assert.equal(await activeFor(PROJECT), 0, "ningún rechazo deja fila");
  });

  await t.test("una caducidad que no está en el futuro se rechaza", async () => {
    for (const expires of ["2020-01-01T00:00:00Z", "now()"]) {
      await assert.rejects(
        () => rpc(db, "authenticated", OWNER,
          expires === "now()"
            ? "public.portal_issue_token($1,'[\"read\"]'::jsonb, now())"
            : "public.portal_issue_token($1,'[\"read\"]'::jsonb, $2::timestamptz)",
          expires === "now()" ? [PROJECT] : [PROJECT, expires]),
        (error) => error.code === "22023" && /expires_at must be in the future/.test(error.message),
      );
    }
    const ok = await rpc(db, "authenticated", OWNER,
      "public.portal_issue_token($1,'[\"read\"]'::jsonb, now() + interval '30 days')", [PROJECT]);
    assert.ok(ok.expires_at, "una caducidad futura sí se acepta");
    await db.query("delete from public.portal_tokens where id=$1", [ok.id]);
  });
  ...
  await t.test("rotar emite el sustituto y revoca el anterior en la misma transacción", async () => {
    ...
    assert.equal(await activeFor(PROJECT), 1, "queda exactamente un enlace vigente");
  ...
  await t.test("dos rotaciones simultáneas no dejan dos sustitutos vigentes", async () => {
    ...
    assert.equal(await activeFor(PROJECT), 1, "un solo enlace vigente tras la carrera");
    await db.query("delete from public.portal_tokens where project_id=$1", [PROJECT]);
  });
  ...
  await t.test("revocar es idempotente y no repite el secreto", async () => {
    ...
    assert.equal(await activeFor(PROJECT), 0);
    await db.query("delete from public.portal_tokens where project_id=$1", [PROJECT]);
  });
