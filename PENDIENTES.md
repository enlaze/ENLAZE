...
- **🟡 SEGURIDAD — retirada de enlaces heredados del portal:** _Revisado 2026-09-24:_ el portal valida el portador mediante RPC, `anon` no puede leer `portal_tokens` ni escribir `project_changes`, y E4 ya aporta ciclo de vida y listado de metadatos. El lote 2 sustituye la lectura directa del panel y retira `SELECT` a `authenticated`. Quedan **8 `projects.access_token` heredados**: sustituirlos uno a uno, verificar la entrega y retirar después la compatibilidad heredada; no conceder permisos modernos en bloque.
