...
interface TierKeywords {
  basica: string[];
  alta: string[];
}

const TIER_KEYWORDS: Record<string, TierKeywords> = {
  pavimentos: {
...
export function classifyQualityTier(
  ...
  if (!keywords) return requestedTier;

  for (const kw of keywords.alta) {
    if (lower.includes(kw)) return "alta";
  }
  for (const kw of keywords.basica) {
    if (lower.includes(kw)) return "basica";
  }
...
const QUALITY_MULTIPLIERS: Record<string, Record<QualityTier, number>> = {
...
export function getQualityMultiplier(chapter: string, tier: QualityTier): number {
  const map = QUALITY_MULTIPLIERS[chapter] || QUALITY_MULTIPLIERS.default;
  return map[tier];
}

// ─── Internal Estimate Database (fallback level 6) ──────────────────────────
 ...
 */
const INTERNAL_PRICE_DB: BasePrice[] = [
  // Albanileria
...
// ─── Price Resolution (single material) ─────────────────────────────────────

/**
 * Resolve price for a single material through the priority chain.
 * Levels 1-4 require database access (passed as pre-fetched data).
 * Level 5 (web search) is handled externally and results passed in.
 * Level 6 is the internal estimate fallback.
 *
...
export function resolveMaterialPrice(
  ...
  const alternatives: PriceAlternative[] = [];
  ...
  if (userPrices && userPrices.length > 0) {
    ...
    if (match) {
      ...
      return buildResult(request, normalized, match.unit_price, match.supplier_name || "Catalogo propio",
        "", "user_catalog", 0.95, now, alternatives);
    }
  }
  ...
  if (enlazePrices && enlazePrices.length > 0) {
    ...
    if (match) {
      ...
      return buildResult(request, normalized, match.unit_price, "Banco ENLAZE",
        "", "enlaze_base", 0.80, now, alternatives);
    }
  }
  ...
  if (n8nPrices && n8nPrices.length > 0) {
    ...
    if (match) {
      ...
      return buildResult(request, normalized, match.value, match.source || "n8n market",
        "", "n8n_market", 0.75, now, alternatives);
    }
  }
  ...
  if (webResults && webResults.length > 0) {
    ...
    return buildResult(request, normalized, selected.price, selected.supplier,
  ...
  if (dbMatch) {
    ...
    return buildResult(request, normalized, price, dbMatch.supplier_ref,
      "", "estimated", 0.40, now, alternatives);
  }

  // Absolute fallback: return zero with very low confidence
  return buildResult(request, normalized, 0, "Sin fuente",
    "", "estimated", 0.10, now, []);
}
 ...
 */
export function resolvePricesForBudget(
  input: BatchPriceInput
): { resolved: ResolvedPrice[]; needsWebSearch: PriceRequest[] } {
  const resolved: ResolvedPrice[] = [];
  const needsWebSearch: PriceRequest[] = [];

  for (const mat of input.materials) {
    const result = resolveMaterialPrice(
      mat,
      input.userPrices,
      input.enlazePrices,
      input.n8nPrices,
...
export function isCacheValid(entry: PriceCacheEntry): boolean {
