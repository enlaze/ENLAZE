...
export type EmailImportance = "critical" | "important" | "normal" | "noise";
const IMPORTANCE_SET = new Set<EmailImportance>(["critical", "important", "normal", "noise"]);

export interface HaikuClassifyItem {
  idx: number;
  from_name: string;
  from_email: string;
  subject: string;
...
/** Minimal sector/persona context so Haiku reasons like the right expert. */
 ...
 */
export async function classifyEmailsWithHaiku(
  items: HaikuClassifyItem[],
  ctx: BusinessContext,
): Promise<HaikuClassifyResult[] | null> {
...
{"items":[{"idx":<number>,"importance":"critical|important|normal|noise","category":"customer|supplier|lead|internal|spam|unknown","reason":"frase corta en español"}]}`;
  ...
  try {
    ...
    const out: HaikuClassifyResult[] = [];
    for (const raw of parsed.items) {
      if (!raw || typeof raw !== "object") continue;
      const r = raw as Record<string, unknown>;
      const idx = typeof r.idx === "number" ? r.idx : Number(r.idx);
      const importance = String(r.importance || "") as EmailImportance;
      if (!Number.isFinite(idx) || !IMPORTANCE_SET.has(importance)) continue;
      out.push({
        idx,
        importance,
