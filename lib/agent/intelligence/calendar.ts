...
export interface UpcomingImportant {
  date: string;
  title: string;
...
export interface CalendarIntel {
  ...
  tomorrow: DayAgenda;
  this_week: WeekStats;
  upcoming_important: UpcomingImportant[];
...
function emptyDay(date: string): DayAgenda {
  return {
    date,
    events: [],
    total_events: 0,
...
export function emptyCalendarIntel(status: CalendarStatus, error: string | null = null): CalendarIntel {
...
function workdayWindow(d: Date): { start: Date; end: Date } {
  const s = new Date(d);
  s.setHours(9, 0, 0, 0);
  const e = new Date(d);
  e.setHours(18, 0, 0, 0);
...
function buildDayAgenda(date: Date, rawEvents: RawEvent[]): DayAgenda {
  ...
  const merged: Array<{ s: Date; eEnd: Date }> = [];
  for (const r of timedRanges) {
    ...
    if (r.s <= last.eEnd) {
      if (r.eEnd > last.eEnd) last.eEnd = r.eEnd;
    } else {
      merged.push({ ...r });
    }
  }

  const free_blocks: FreeBlock[] = [];
  let cursor = workStart;
  for (const r of merged) {
    const inStart = r.s < workStart ? workStart : r.s;
    const inEnd = r.eEnd > workEnd ? workEnd : r.eEnd;
    if (inEnd > inStart) busyMs += inEnd.getTime() - inStart.getTime();
    ...
    if (r.eEnd > cursor) cursor = r.eEnd > workEnd ? workEnd : r.eEnd;
  }
  if (cursor < workEnd) {
...
function isImportant(ev: RawEvent): { important: boolean; why: string } {
...
export async function fetchCalendarIntel(accessToken: string): Promise<CalendarIntel> {
  const t0 = Date.now();

  // 1) Discover calendars the user actually wants surfaced.
  const calList = await calGet<{ items?: CalendarListEntry[] }>(
  ...
  if (!calList.ok) {
    ...
    if (calList.status === 429) return emptyCalendarIntel("rate_limited", "Calendar 429");
    return emptyCalendarIntel("api_error", `Calendar list ${calList.status} ${calList.statusText}`);
  }
  const visible = (calList.body.items || []).filter(
    (c) => !c.hidden && (c.selected || c.primary),
  );
  if (visible.length === 0) {
    visible.push({ id: "primary", summary: "primary", primary: true } as CalendarListEntry);
  }
  ...
  for (const cal of visible) {
    ...
    const r = await calGet<{ items?: RawEvent[] }>(url, accessToken);
    if (!r.ok) {
  ...
  let busiest_day: WeekStats["busiest_day"] = null;
  let quietest_day: WeekStats["quietest_day"] = null;
  ...
  const upcoming_important: UpcomingImportant[] = [];
  for (const ev of allEvents) {
    const startTs = ev.start?.dateTime || ev.start?.date;
    if (!startTs) continue;
    const s = new Date(startTs);
  ...
  const recurringBuckets = new Map<string, { description: string; occurrences: number }>();
  ...
  console.log(
    `${LOG} calendars=${visible.length} events=${allEvents.length} today=${today.total_events} tomorrow=${tomorrow.total_events} upcoming_important=${upcoming_important.length} elapsed_ms=${Date.now() - t0}`,
  );
