...
export const MATERIAL_SPECS: MaterialSpec[] = [
  ...
  { id: "multilayer-pipe-16-50", name: "Tubo multicapa azul con aislamiento 16 mm rollo 50 m", specification: "Diámetro 16 mm; rollo de 50 m; uso agua fría, caliente y calefacción", unit: "rollos", unit_price: 58.0, provider_id: "obramat", qtyFn: q => Math.ceil((q.bathroomsCount + (q.kitchenIncluded ? 1 : 0)) * 1.2), chapter: "fontaneria" },
  { id: "pvc-drain-110-3", name: "Tubo 110 mm 3 m PVC compacto", specification: "Tubería PVC de evacuación compacta; diámetro exterior 110 mm; barra de 3 m; UNE-EN 1329", unit: "ud", unit_price: 7.95, provider_id: "obramat", qtyFn: q => Math.ceil(q.bathroomsCount * 3 + (q.kitchenIncluded ? 2 : 0)), chapter: "fontaneria" },
  ...
  { id: "multilayer-manifold-four-16", name: "Colector 1 pulgada x 16 mm 4 salidas 1/2 pulgada multicapa", specification: "Colector multicapa; entrada de 1 pulgada; cuatro salidas de 1/2 pulgada para tubo de 16 mm; euroconos incluidos", unit: "ud", unit_price: 15.61, provider_id: "obramat", qtyFn: q => Math.max(q.bathroomsCount + (q.kitchenIncluded ? 1 : 0), 1), chapter: "fontaneria" },
  { id: "basin-bottle-trap-32", name: "Sifón lavabo botella extensible 1 1/4 pulgadas blanco", specification: "Sifón botella extensible; diámetro 32 mm (1 1/4 pulgadas); salida vertical", unit: "ud", unit_price: 3.54, provider_id: "obramat", qtyFn: q => q.bathroomsCount, chapter: "fontaneria" },
  ...
  { id: "sink-trap-40", name: "Sifón fregadero botella extensible 1 1/2 pulgadas blanco Girpi", specification: "Sifón botella para fregadero de un seno; conexión de 1 1/2 pulgadas; salida horizontal de 40 mm", unit: "ud", unit_price: 6.60, provider_id: "obramat", qtyFn: q => q.kitchenIncluded ? 1 : 0, chapter: "fontaneria" },
  { id: "sink-basket-waste", name: "Válvula fregadero 115 mm con vástago blanco Girpi", specification: "Válvula de desagüe con cesta; diámetro 115 mm; destino fregadero; acabado blanco", unit: "ud", unit_price: 6.60, provider_id: "obramat", qtyFn: q => q.kitchenIncluded ? 1 : 0, chapter: "fontaneria" },

  // Electricidad: cuadro, protecciones y mecanismos se verifican por separado.
  { id: "cable-h07vk-25-100", name: "Hilo PVC H07V-K 2.5 mm2 gris 100 m", specification: "Sección 2,5 mm²; rollo 100 m; conductor flexible gris", unit: "rollos", unit_price: 30.25, provider_id: "obramat", qtyFn: q => Math.ceil(q.electricalPointsEstimated / 30), chapter: "electricidad" },
  { id: "panel-box-12-flush", name: "Cuadro eléctrico para empotrar en vivienda de 12 módulos Schneider", specification: "Instalación empotrada; capacidad 12 módulos; marco y puerta blancos", unit: "ud", unit_price: 14.52, provider_id: "obramat", qtyFn: () => 1, chapter: "electricidad" },
  { id: "rcd-2p-40a-30ma", name: "Diferencial 2P 40A 30mA Schneider", specification: "2 polos; intensidad 40 A; sensibilidad 30 mA; clase AC", unit: "ud", unit_price: 21.0, provider_id: "obramat", qtyFn: () => 1, chapter: "electricidad" },
  { id: "mcb-1pn-16a-c", name: "Automático magnetotérmico 1P+N 1 módulo 16A Solera", specification: "1P+N; un módulo; intensidad 16 A; curva C; poder de corte 6 kA", unit: "ud", unit_price: 6.06, provider_id: "obramat", qtyFn: q => Math.max(Math.ceil(q.electricalPointsEstimated / 14), 3), chapter: "electricidad" },
  { id: "surge-protection-2p", name: "Sobretensión transitoria 1P+N Legrand", specification: "Protector de sobretensiones transitorias tipo 2; 1P+N; dos módulos; uso residencial", unit: "ud", unit_price: 47.0, provider_id: "obramat", qtyFn: () => 1, chapter: "electricidad" },
  { id: "socket-schuko-16a-white", name: "Schuko serie Europa blanco", specification: "Base 2P+T Schuko; 16 A y 250 V; instalación empotrada; color blanco", unit: "ud", unit_price: 2.79, provider_id: "obramat", qtyFn: q => Math.max(Math.round(q.electricalPointsEstimated * 0.65), 1), chapter: "electricidad" },
  { id: "switch-one-way-10a-white", name: "Conmutador serie Europa blanco", specification: "Conmutador utilizable como interruptor; 10 A y 250 V; instalación empotrada; color blanco", unit: "ud", unit_price: 2.38, provider_id: "obramat", qtyFn: q => Math.max(Math.round(q.electricalPointsEstimated * 0.35), 1), chapter: "electricidad" },
  { id: "led-surface-20w-4000k", name: "Downlight LED superficie circular blanco 20W luz neutra", specification: "Potencia 20 W; temperatura 4000 K; protección IP20; montaje en superficie", unit: "ud", unit_price: 4.74, provider_id: "obramat", qtyFn: q => Math.max(Math.round(q.floorArea / 6), 4), chapter: "electricidad" },

  // Acabados: se elimina la alternativa cerámico/laminado y se define una solución completa.
  { id: "porcelain-wall-60-60", name: "Suelo porcelánico 60x60 cm London grafito", specification: "Porcelánico rectificado; formato 60x60 cm; acabado mate; apto para interior y zonas húmedas", unit: "m2", unit_price: 12.96, provider_id: "obramat", qtyFn: q => Math.ceil(q.wetWallArea * 1.1), chapter: "revestimientos" },
  ...
  { id: "laminate-underlay-5", name: "Base suelo laminado", specification: "Base de polietileno de baja densidad; espesor 2,5 mm; rollo de 30 m²; reducción acústica 17 dB", unit: "m2", unit_price: 0.83, provider_id: "obramat", qtyFn: q => Math.ceil(q.pavementArea * 1.1), chapter: "pavimentos" },
  ...
  { id: "primer-water-15", name: "Fondo fijador acrílico 15 L blanco", specification: "Imprimación blanca en base agua; consolida y regulariza placas de yeso; envase 15 L", unit: "cubos", unit_price: 34.0, provider_id: "obramat", qtyFn: q => Math.ceil((q.wallPaintArea + q.ceilingArea) / 120), chapter: "pintura" },
  { id: "interior-repair-putty-15", name: "Plaste en polvo capa gruesa renovación 2h 15 kg", specification: "Uso interior; renovación y reparación; capa máxima 5 mm; saco de 15 kg", unit: "sacos", unit_price: 7.03, provider_id: "obramat", qtyFn: q => Math.max(Math.ceil(q.wallPaintArea / 100), 1), chapter: "pintura" },
  { id: "masking-tape-50-50", name: "Cinta de pintor exterior 50 mm 50 m", specification: "Cinta de enmascarar para pintura exterior; ancho 50 mm; longitud 50 m", unit: "ud", unit_price: 5.20, provider_id: "obramat", qtyFn: q => Math.max(Math.ceil(q.floorArea / 25), 2), chapter: "pintura" },
  { id: "protective-film-4-5", name: "Plástico cubretodo fino reciclado 4x5 m", specification: "Polietileno reciclado; formato 4x5 m; protección y enmascaramiento durante el pintado", unit: "ud", unit_price: 0.65, provider_id: "obramat", qtyFn: q => Math.max(Math.ceil(q.floorArea / 20), 1), chapter: "pintura" },
  { id: "paint-roller-22", name: "Rodillo superficies lisas microfibra seda 22 cm", specification: "Ancho 22 cm; microfibra; apto para pinturas plásticas sobre superficies lisas", unit: "ud", unit_price: 5.20, provider_id: "obramat", qtyFn: q => Math.max(Math.ceil(q.floorArea / 80), 1), chapter: "pintura" },
  { id: "paint-brush-40", name: "Brocha prensada fibra sintética nº10", specification: "Fibra sintética; ancho 34 mm; apta para recortar con todo tipo de pinturas", unit: "ud", unit_price: 4.10, provider_id: "obramat", qtyFn: q => Math.max(Math.ceil(q.floorArea / 100), 1), chapter: "pintura" },
  { id: "paint-tray-16", name: "Cubeta de plástico 16 L con rejilla", specification: "Capacidad 16 L; rejilla incluida; apta para rodillos de 22 y 25 cm", unit: "ud", unit_price: 3.54, provider_id: "obramat", qtyFn: q => Math.max(Math.ceil(q.floorArea / 100), 1), chapter: "pintura" },

  // Sanitarios: cada aparato y cada grifería conservan su precio independiente.
  { id: "toilet-compact-dual-white", name: "Pack WC compacto salida dual", specification: "Pack completo; adosado a pared; salida dual; porcelana blanca; asiento amortiguado y cisterna 3/4,5 L incluidos", unit: "ud", unit_price: 93.0, provider_id: "obramat", qtyFn: q => q.bathroomsCount, chapter: "sanitarios" },
  { id: "countertop-basin-round-400", name: "Lavabo sobreponer redondo 40 cm blanco", specification: "Instalación sobre encimera; diámetro 40 cm; porcelana blanca", unit: "ud", unit_price: 43.0, provider_id: "obramat", qtyFn: q => q.bathroomsCount, chapter: "sanitarios" },
  ...
  { id: "shower-tray-resin-120-80", name: "Plato de ducha resina rectangular blanco 80x120x3 cm", specification: "Resina; 80x120x3 cm; antideslizante clase 3; color blanco; válvula y rejilla incluidas", unit: "ud", unit_price: 105.0, provider_id: "obramat", qtyFn: q => q.bathroomsCount, chapter: "sanitarios" },
  { id: "shower-screen-sliding-120", name: "Mampara de ducha frontal corredera Denver Pro 120 cm cromo", specification: "Frontal; corredera; ancho 120 cm; vidrio transparente de 6 mm; perfil cromado; altura 195 cm", unit: "ud", unit_price: 160.0, provider_id: "obramat", qtyFn: q => q.bathroomsCount, chapter: "sanitarios" },
  ...
  { id: "door-block-white-725-right", name: "Puerta interior en block lisa lacada blanca ciega 72.5 cm derecha", specification: "Block completo; hoja lisa ciega lacada blanca; ancho 72,5 cm; mano derecha", unit: "ud", unit_price: 85.0, provider_id: "obramat", qtyFn: q => Math.floor(q.doorsEstimated / 2), chapter: "carpinteria_interior" },

  // Impermeabilización y sellado.
  { id: "waterproof-membrane-1-10", name: "Lámina geotextil 1.5x2.5 m Jimten", specification: "Lámina impermeable de polietileno geotextil; formato 1,5x2,5 m; espesor 0,4 mm; apta para zonas húmedas", unit: "ud", unit_price: 19.76, provider_id: "obramat", qtyFn: q => Math.max(Math.ceil((q.wetWallArea * 0.4) / 3.75), 1), chapter: "impermeabilizacion" },
  { id: "sanitary-silicone-clear-300", name: "Silicona neutra para baños y cocinas 280 ml transparente", specification: "Silicona neutra para zonas húmedas; transparente; cartucho de 280 ml; resistente al ennegrecimiento", unit: "ud", unit_price: 4.41, provider_id: "obramat", qtyFn: q => Math.max(q.bathroomsCount * 3, 4), chapter: "sanitarios" },
