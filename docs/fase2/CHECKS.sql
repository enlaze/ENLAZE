...
-- ─────────────────────────────────────────────────────────────────────────────────────
-- BLOQUE 11 · 20260901120000_budget_items_sort_order.sql (FASE 2E-2)
--
-- NINGUNA CONSULTA LLEVA UN VALOR ESPERADO VOLATIL. No hay recuentos de filas ni
-- hashes clavados. El criterio es o bien PRE == POST, o bien un invariante
-- estructural cuyo valor correcto es 0 / true por definicion, no por medicion.

-- ── 11.A · BASELINE. Volcado por filas. ─────────────────────────────────────────────
-- Esta consulta NO vive en un fichero aparte: es la de aqui abajo, delimitada por los
-- marcadores CHECK_11A. Se extrae a un temporal con el `awk` documentado en 11.A1 y se
-- ejecuta TRES veces con ese mismo temporal: 11.A1, 11.A2 y 11.C. Ejecutar las tres
-- desde el mismo fichero es lo que garantiza que se compara exactamente lo mismo.
--
-- Por filas y no agregado: si algo diverge, el diff dice QUE fila y en que sentido.
-- La huella es to_jsonb(bi) - 'sort_order': la fila ENTERA menos esa clave. No enumera
-- columnas, asi que cubre las 22 de hoy y las que se anadan manana sin envejecer.
-- BEGIN CHECK_11A
select bi.id::text,
...
-- ── 11.A1 / 11.A2 · PUERTA DE QUIESCENCIA. ANTES de aplicar. ────────────────────────
-- PASO 0. Extraer el SELECT de 11.A a un temporal. Desde la raiz del worktree:
--
--   awk '/^-- BEGIN CHECK_11A$/{emitir=1;next}/^-- END CHECK_11A$/{emitir=0}emitir' \
--     docs/fase2/CHECKS.sql > /tmp/2e_checks_11a.sql
--   test -s /tmp/2e_checks_11a.sql
--
-- El `test -s` no es decorativo: si un renombrado futuro rompe los marcadores, el `awk`
-- devuelve 0 y escribe un fichero VACIO, y psql sobre un fichero vacio termina con
-- codigo 0 y salida vacia. Sin esa comprobacion, los tres diffs de mas abajo darian
-- PASS sin haber medido nada.
--
-- Dos ejecuciones consecutivas de 11.A separadas por >= 60 s, con la aplicacion en el
-- estado en que se va a desplegar:
--
--   psql "$DB_URL" -X -v ON_ERROR_STOP=1 -At -F'|' \
--     -f /tmp/2e_checks_11a.sql > /tmp/2e_pre1.txt
--
--   sleep 60
--
--   psql "$DB_URL" -X -v ON_ERROR_STOP=1 -At -F'|' \
--     -f /tmp/2e_checks_11a.sql > /tmp/2e_pre2.txt
--
--   diff -u /tmp/2e_pre1.txt /tmp/2e_pre2.txt
--
-- `-X` ignora el ~/.psqlrc de quien despliega, y `-v ON_ERROR_STOP=1` hace que un error
-- pare con codigo distinto de 0 en vez de dejar una salida truncada que el diff
-- interpretaria como una diferencia real.
--
-- CRITERIO: salida vacia, codigo 0.
-- Si difiere, la tabla NO esta en reposo y la linea base no es estable. NO se despliega:
-- se identifica el escritor, se detiene o se espera, y se repite la puerta desde cero.
-- Desplegar un cambio de esquema sobre una tabla en movimiento hace que 11.C no pueda
-- distinguir despues entre trafico y dano.

-- ── 11.B · ENSAYO EN SECO DEL ALGORITMO REAL. ANTES. Sin escribir nada. ─────────────
-- Reproduce LITERALMENTE las CTE `elegibles`, `posiciones` y la asignacion que la
-- migracion materializa en `tmp_2e_asignacion`, incluida la reconstruccion desde
-- `wizard_state->'partidas'` y `wizard_state->'materials'`. No sustituye el orden real
-- por `(created_at, id)`: eso seria un orden total, pero no seria ESTE algoritmo, y el
-- ensayo dejaria de ensayar lo unico que importa.
with elegibles as (
  select bi.budget_id
    from public.budget_items bi
  ...
  select bi.id,
         ...
         (pos.clave is not null) as emparejada
    from public.budget_items bi
  ...
  select budget_id,
         count(*)                                            as n,
         count(distinct nuevo_sort_order)                     as d,
...
select
  -- CRITERIOS DE PASS (fijos, no volatiles)
  ((select count(*) from filas_elegibles) = (select count(*) from asignacion))
                                                                as cuadra_total,
  ((select count(*) from asignacion) = (select count(distinct id) from asignacion))
                                                                as ids_unicos,
  (select count(*) from filas_elegibles f
    where not exists (select 1 from asignacion a where a.id = f.id))
                                                                as ids_sin_asignar,
  (select count(*) from porpres
    where nulos <> 0 or negativos <> 0 or mn <> 0 or mx <> n - 1 or d <> n)
                                                                as presupuestos_mal,
  -- INFORMATIVOS (se anotan en el runbook; NUNCA son criterio)
  (select count(*) from filas_elegibles)                        as i_filas_elegibles,
  (select count(*) from asignacion)                             as i_filas_asignadas,
  (select count(*) from asignacion where emparejada)            as i_emparejadas,
  (select count(*) from asignacion where not emparejada)        as i_no_emparejadas;
-- CRITERIO: cuadra_total = true, ids_unicos = true, ids_sin_asignar = 0,
--           presupuestos_mal = 0.
-- Los cuatro i_* se anotan y no se comparan contra nada. Un presupuesto nuevo cambia
-- los cuatro sin que nada este mal; una fila sin emparejar con wizard_state no es un
-- fallo, cae al final ordenada por id, que es el comportamiento declarado.

-- ── 11.C · INVARIANTE ABSOLUTA. DESPUES de aplicar. ─────────────────────────────────
-- Con el MISMO /tmp/2e_checks_11a.sql que se uso en 11.A1 y 11.A2, sin regenerarlo:
--
--   psql "$DB_URL" -X -v ON_ERROR_STOP=1 -At -F'|' \
--     -f /tmp/2e_checks_11a.sql > /tmp/2e_post.txt
--
--   diff -u /tmp/2e_pre2.txt /tmp/2e_post.txt
--
-- CRITERIO DE PASS: salida vacia, codigo 0. Y SOLO eso.
--
-- CUALQUIER diferencia -alta, baja o cambio de contenido- deja la verificacion
-- INCONCLUSA POR CONCURRENCIA. No es un pass, no es un fallo, y no se interpreta sobre
-- la marcha. Una linea '>' con un id nuevo NO es "trafico normal": durante un cambio de
-- esquema significa que la puerta de quiescencia 11.A1/11.A2 no se sostuvo, y por tanto
-- que este diff ya no puede distinguir una insercion legitima de una fila creada por un
-- camino inesperado.
--
-- PROCEDIMIENTO ANTE UN DIFF NO VACIO, en este orden y sin saltarse pasos:
--   1. DETENER la validacion. No continuar con 11.D-11.F ni dar por bueno el despliegue.
--   2. IDENTIFICAR la operacion: para cada id que aparece, desaparece o cambia, buscar
--      quien lo escribio (logs de la aplicacion, `pg_stat_activity`, el propio
--      `created_at`/`updated_at` de la fila) y por que camino.
--   3. DECIDIR con esa informacion, no sin ella. Si la operacion es una escritura de la
--      aplicacion, se detiene el escritor. Si no se explica, se revierte con el
--      BLOQUE 0-2E de ROLLBACK.sql.
--   4. REPETIR la validacion entera desde 11.A1 sobre una linea base estable.
-- En ningun caso se ignora el diff.

-- ── 11.D · ORDEN 0..N-1 EN TODOS LOS PRESUPUESTOS. DESPUES. ─────────────────────────
select count(*) as presupuestos_mal
  from (
    select budget_id,
           ...
           count(distinct sort_order) as d
      from public.budget_items
     group by budget_id
  ) g
 where g.mn <> 0 or g.mx <> g.n - 1 or g.d <> g.n;
-- CRITERIO: 0. Dinamico: no sabe cuantos presupuestos hay ni de que tamano.

-- ── 11.E · CONTRATO DE ESQUEMA. DESPUES. ────────────────────────────────────────────
-- NO se usa information_schema.columns.column_default = '0'. Postgres puede
-- representar ese default como `0`, `0::integer`, `(0)::integer` o `'0'::integer` segun
-- como se escribiera, y una comparacion de cadena falla o pasa por accidente. Se lee del
-- catalogo con pg_get_expr y se compara SEMANTICAMENTE, como numero.
select
  -- NOT NULL: del catalogo, no de information_schema.
  a.attnotnull                                                   as not_null_ok,
  -- DEFAULT: expresion cruda, para el log.
  pg_get_expr(ad.adbin, ad.adrelid)                              as default_expr,
  -- DEFAULT: comparacion semantica. Se quita un cast de sufijo y un parentesis
  -- envolvente, y se compara como numeric. `0`, `0::integer`, `(0)::integer` y
  -- `(0)::numeric` dan todos true; `1` o `NULL` dan false.
  (nullif(regexp_replace(
     regexp_replace(pg_get_expr(ad.adbin, ad.adrelid), '::[a-zA-Z0-9_ ]+$', ''),
     '^\((.*)\)$', '\1'), '')::numeric = 0)                       as default_es_cero
  from pg_attribute a
  left join pg_attrdef ad on ad.adrelid = a.attrelid and ad.adnum = a.attnum
 where a.attrelid = 'public.budget_items'::regclass
   and a.attname  = 'sort_order';
-- CRITERIO: not_null_ok = true, default_es_cero = true.
-- (Medido hoy sobre produccion, ANTES de aplicar: attnotnull = false, default_expr = '0'.
--  Es el estado de partida, no un valor esperado.)

-- 11.E bis · Las dos constraints: que existan, que esten VALIDADAS, y que su definicion
-- sea exactamente la esperada. `convalidated` importa: una constraint NOT VALID existe
-- en el catalogo y no garantiza nada sobre las filas que ya estaban.
select c.conname,
       c.contype,
       c.convalidated,
       pg_get_constraintdef(c.oid)                                as definicion,
 ...
 where c.conrelid = 'public.budget_items'::regclass
   and c.conname in ('ck_budget_items_sort_order_non_negative',
                     'uq_budget_items_budget_id_sort_order')
 order by c.conname;
-- CRITERIO: exactamente 2 filas, ambas con convalidated = true, y
--   ck_budget_items_sort_order_non_negative : contype = 'c'
--        definicion_norm = 'checksort_order>=0'
--   uq_budget_items_budget_id_sort_order    : contype = 'u'
--        definicion_norm = 'uniquebudget_id,sort_order'
-- Se compara la forma NORMALIZADA (minusculas, sin espacios ni parentesis) porque
-- pg_get_constraintdef anade parentesis y casts segun version y tipo. La columna
-- `definicion` cruda se imprime siempre, para que quede en el log lo que habia de
-- verdad y no solo un booleano.

-- 11.E ter · Un solo indice sobre (budget_id, sort_order): el que crea la UNIQUE por
-- debajo, que es exactamente el que necesita el ORDER BY de los lectores. Un segundo
-- seria el mismo indice dos veces, con el coste de escritura duplicado.
select count(*) as indices
  from pg_indexes
 where schemaname = 'public' and tablename = 'budget_items'
   and indexdef ilike '%(budget_id, sort_order)%';
-- CRITERIO: 1.

-- ── 11.F · FORMA DE LA RPC. DESPUES. ────────────────────────────────────────────────
--
-- `pg_proc.prosrc` conserva el cuerpo de la funcion VERBATIM, comentarios incluidos. Un
-- `like` sobre `prosrc` a secas encuentra por tanto lo que solo aparece dentro de un
-- comentario: el cuerpo de esta RPC menciona `item->>'sort_order'` en un comentario que
-- explica precisamente que NO lo lee, y esa sola mencion bastaria para dar un falso
-- positivo. Por eso se descartan las lineas de comentario ANTES de buscar el patron, y
-- las tres comprobaciones se hacen solo sobre el codigo ejecutable reconstruido.
with funcion as (
  select p.prosrc
    from pg_proc p
   where p.oid =
     'public.update_budget_with_items(uuid, jsonb, jsonb)'::regprocedure
), codigo as (
  select string_agg(l.linea, E'\n' order by l.numero) as prosrc_ejecutable
    from funcion f
    cross join lateral regexp_split_to_table(f.prosrc, E'\r?\n')
      with ordinality as l(linea, numero)
   where l.linea !~ '^[[:space:]]*--'
)
select (prosrc_ejecutable like '%with ordinality%')
         as usa_ordinalidad,
       (prosrc_ejecutable like '%(ordinality - 1)::integer%')
         as base_cero,
       (prosrc_ejecutable not like '%item->>''sort_order''%')
         as ignora_sort_order_del_json
  from codigo;
-- CRITERIO: true | true | true. La posicion sale de la ordinalidad del array, no de un
-- campo del JSON que podria venir repetido o con huecos.

-- ── 11.G · CONTINUIDAD FINANCIERA Y CANONICA. ANTES **Y** DESPUES. ──────────────────
--
...
-- da igual". La invariante real de un reordenamiento no es un recuento concreto de
-- filas: es que NADA de lo que se mide aqui cambie, sea cual sea su valor.
--
...
-- BLOQUE 0-2E de ROLLBACK.sql.
--
-- PROCEDIMIENTO, sin comparar hashes a ojo. El SELECT es el de aqui abajo, delimitado
-- por los marcadores CHECK_11G. Desde la raiz del worktree:
--
--   awk '/^-- BEGIN CHECK_11G$/{emitir=1;next}/^-- END CHECK_11G$/{emitir=0}emitir' \
--     docs/fase2/CHECKS.sql > /tmp/2e_checks_11g.sql
--   test -s /tmp/2e_checks_11g.sql
--
--   psql "$DB_URL" -X -v ON_ERROR_STOP=1 -At -F'|' \
--     -f /tmp/2e_checks_11g.sql > /tmp/2e_financial_pre.txt
--
--   ... aplicar la migracion ...
--
--   psql "$DB_URL" -X -v ON_ERROR_STOP=1 -At -F'|' \
--     -f /tmp/2e_checks_11g.sql > /tmp/2e_financial_post.txt
--
--   diff -u /tmp/2e_financial_pre.txt /tmp/2e_financial_post.txt
--   # PASS es salida vacia, codigo 0
--
-- El PRE se mide con el fichero ya extraido y el POST se mide con ESE MISMO fichero: si
-- se regenerase entre medias se estaria comparando el resultado de dos consultas
-- potencialmente distintas y el diff dejaria de significar nada.
--
...
--                     manana, sin enumerar ninguna. Ojo, NO es comparable con la salida
--                     de 11.A: alli se vuelca una linea por fila, sin agregar, y aqui
--                     se agrega en un solo md5 ordenado por id. Cada una se compara
--                     consigo misma.
