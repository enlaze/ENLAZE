...
export default function EmailsPage() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    await supabase.from("messages").insert({
      ...
      status,
      sent_at: json.success ? new Date().toISOString() : null,
    });
            ...
            <form onSubmit={handleSend} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {templates.map((t, i) => (
                    <button key={i} type="button" onClick={() => applyTemplate(t)} className="px-3 py-1.5 rounded-lg border border-navy-200 text-xs font-medium text-navy-700 hover:bg-navy-50 transition-colors">{t.name}</button>
                  ))}
                </div>
              </div>
              <div>
        ...
        <div className="lg:col-span-2">
          <div className="rounded-2xl border border-navy-100 bg-white shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-navy-100">
              <h2 className="font-bold text-navy-900">Historial de emails</h2>
            </div>
            {messages.length === 0 ? (
              <div className="p-12 text-center">
                <p className="text-4xl mb-4">📧</p>
                <h3 className="text-lg font-bold text-navy-900">Sin emails todavia</h3>
                <p className="mt-2 text-navy-600">Envia tu primer email automatico</p>
              </div>
            ) : (
