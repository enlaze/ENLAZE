...
const statusMap: Record<string, { label: string; color: string }> = {
  ...
  issued: { label: "Emitida", color: "bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300" },
  sent: { label: "Enviada", color: "bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300" },
  paid: { label: "Cobrada", color: "bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-300" },
  overdue: { label: "Vencida", color: "bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-300" },
  cancelled: { label: "Anulada", color: "bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400" },
  rectified: { label: "Rectificada", color: "bg-orange-50 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300" },
};
...
const inputCls = "w-full bg-white dark:bg-zinc-800 text-navy-900 dark:text-white rounded-lg px-4 py-2 border border-navy-200 dark:border-zinc-700 focus:border-brand-green focus:outline-none text-sm";
...
export default function IssuedInvoiceDetailPage() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-64"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-brand-green"></div></div>;
  }
  if (!invoice) return null;

  const st = statusMap[invoice.status] || { label: invoice.status, color: "bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300" };
  const gross = lineForm.quantity * lineForm.unit_price;
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <Breadcrumbs
        ...
        items={[
          ...
          { label: "Facturas emitidas", href: "/dashboard/issued-invoices" },
          { label: invoice.invoice_number },
        ]}
      />
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-navy-900 dark:text-white font-mono">{invoice.invoice_number}</h1>
          <p className="text-sm text-navy-500 dark:text-zinc-400">Emitida: {fmtDate(invoice.issue_date)}{invoice.due_date ? ` · Vto: ${fmtDate(invoice.due_date)}` : ""}</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap print:hidden">
          ...
          <select value={invoice.status} onChange={(e) => handleStatusChange(e.target.value)} disabled={savingStatus}
            className="bg-white dark:bg-zinc-800 text-navy-900 dark:text-white rounded-lg px-3 py-1.5 text-sm border border-navy-200 dark:border-zinc-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-white dark:bg-zinc-900 border border-navy-100 dark:border-zinc-800 shadow-sm dark:shadow-none rounded-xl p-4">
          ...
          <p className="text-sm font-medium text-navy-900 dark:text-white">{invoice.client_name}</p>
          {invoice.client_nif && <p className="text-xs text-navy-500 dark:text-zinc-500">NIF: {invoice.client_nif}</p>}
          {invoice.client_address && <p className="text-xs text-navy-500 dark:text-zinc-500">{invoice.client_address}</p>}
          {invoice.client_email && <p className="text-xs text-navy-500 dark:text-zinc-500">{invoice.client_email}</p>}
        </div>
      </div>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4 print:hidden">
          <h3 className="text-sm font-semibold text-brand-green uppercase tracking-wider">Líneas ({lines.length})</h3>
          {invoice.status === "draft" && (
            <button onClick={() => { setLineForm({ description: "", unit: "ud", quantity: 1, unit_price: 0, discount_percent: 0 }); setEditingLineId(null); setShowLineForm(!showLineForm); }}
              className="px-4 py-2 bg-brand-green text-white rounded-lg text-sm font-medium hover:opacity-90 transition">
              + Añadir línea
            </button>
          )}
        </div>
        ...
        {showLineForm && (
          <div className="bg-white dark:bg-zinc-900 rounded-xl p-5 mb-4 border border-navy-100 dark:border-zinc-800 shadow-sm dark:shadow-none print:hidden">
            <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
              <div className="md:col-span-3">
                <label className="block text-xs text-navy-500 dark:text-zinc-400 mb-1">Descripción *</label>
              ...
              <div>
                <label className="block text-xs text-navy-500 dark:text-zinc-400 mb-1">Ud.</label>
                <select value={lineForm.unit} onChange={(e) => setLineForm({ ...lineForm, unit: e.target.value })} className={inputCls}>
                  {unitOptions.map((u) => <option key={u} value={u}>{u}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs text-navy-500 dark:text-zinc-400 mb-1">Cantidad</label>
              ...
              <div>
                <label className="block text-xs text-navy-500 dark:text-zinc-400 mb-1">Precio ud.</label>
              ...
              <div>
                <label className="block text-xs text-navy-500 dark:text-zinc-400 mb-1">Dto. %</label>
            ...
            <div className="flex items-center justify-between mt-4">
              <p className="text-sm text-navy-500 dark:text-zinc-400">Total línea: <strong className="text-navy-900 dark:text-white">{eur(linePreview)}</strong></p>
              <div className="flex gap-3">
                <button onClick={() => { setShowLineForm(false); setEditingLineId(null); }} className="px-4 py-2 text-sm text-navy-500 dark:text-zinc-400">Cancelar</button>
                <button onClick={handleSaveLine} disabled={savingLine}
                  className="px-5 py-2 bg-brand-green text-white rounded-lg text-sm font-medium hover:opacity-90 transition disabled:opacity-50">
        ...
        <div className="bg-white dark:bg-zinc-900 border border-navy-100 dark:border-zinc-800 shadow-sm dark:shadow-none rounded-xl overflow-hidden">
          {lines.length === 0 ? (
            <div className="p-8 text-center"><p className="text-navy-500 dark:text-zinc-500">Añade líneas a esta factura.</p></div>
          ) : (
            <>
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-navy-100 dark:border-zinc-800">
                    <th className="px-4 py-3 text-left text-xs font-semibold text-navy-500 dark:text-zinc-400 uppercase w-8">#</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-navy-500 dark:text-zinc-400 uppercase">Concepto</th>
                    <th className="px-4 py-3 text-center text-xs font-semibold text-navy-500 dark:text-zinc-400 uppercase">Ud.</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-navy-500 dark:text-zinc-400 uppercase">Cant.</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-navy-500 dark:text-zinc-400 uppercase">Precio</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-navy-500 dark:text-zinc-400 uppercase">Dto.</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-navy-500 dark:text-zinc-400 uppercase">Total</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-navy-500 dark:text-zinc-400 uppercase print:hidden">Acc.</th>
                  </tr>
                </thead>
                <tbody>
                  {lines.map((l, idx) => (
                    <tr key={l.id} className="border-b border-navy-100 dark:border-zinc-800 hover:bg-navy-50/60 dark:hover:bg-zinc-800/40 transition">
                      <td className="px-4 py-3 text-navy-500 dark:text-zinc-500">{idx + 1}</td>
                      <td className="px-4 py-3 text-navy-900 dark:text-white">{l.description}</td>
                      <td className="px-4 py-3 text-center text-navy-500 dark:text-zinc-400">{l.unit}</td>
                      <td className="px-4 py-3 text-right text-navy-700 dark:text-zinc-300">{Number(l.quantity).toLocaleString("es-ES")}</td>
                      <td className="px-4 py-3 text-right text-navy-700 dark:text-zinc-300">{eur(l.unit_price)}</td>
                      <td className="px-4 py-3 text-right text-navy-500 dark:text-zinc-400">{l.discount_percent > 0 ? `${l.discount_percent}%` : "—"}</td>
                      <td className="px-4 py-3 text-right font-medium text-navy-900 dark:text-white">{eur(l.total)}</td>
              ...
              <div className="border-t border-navy-100 dark:border-zinc-800 p-4">
                <div className="flex justify-end">
                  <div className="text-right text-sm space-y-1 min-w-[250px]">
                    <div className="flex justify-between"><span className="text-navy-500 dark:text-zinc-400">Subtotal</span><span className="text-navy-900 dark:text-white">{eur(invoice.subtotal)}</span></div>
                    <div className="flex justify-between"><span className="text-navy-500 dark:text-zinc-400">IVA ({invoice.iva_percent}%)</span><span className="text-navy-900 dark:text-white">{eur(invoice.iva_amount)}</span></div>
                    {Number(invoice.irpf_percent) > 0 && (
                      <div className="flex justify-between"><span className="text-red-600 dark:text-red-400">IRPF (-{invoice.irpf_percent}%)</span><span className="text-red-600 dark:text-red-400">-{eur(invoice.irpf_amount)}</span></div>
                    )}
                    <div className="flex justify-between pt-2 border-t border-navy-100 dark:border-zinc-800">
                      <span className="text-navy-900 dark:text-white font-bold text-base">TOTAL</span>
                      <span className="text-brand-green font-bold text-base">{eur(invoice.total)}</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {invoice.verifactu_registered && (
        <div className="bg-white dark:bg-zinc-900 rounded-xl p-5 mb-6 border border-emerald-500/20 shadow-sm dark:shadow-none">
          <h3 className="text-sm font-semibold text-brand-green uppercase tracking-wider mb-3">Verifactu</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              <p className="text-xs text-navy-500 dark:text-zinc-400 mb-1">Hash SHA-256</p>
              <p className="text-xs text-navy-700 dark:text-zinc-300 font-mono break-all bg-navy-50 dark:bg-zinc-800 rounded p-2">{invoice.verifactu_hash || "—"}</p>
            </div>
            <div>
              <p className="text-xs text-navy-500 dark:text-zinc-400 mb-1">Hash anterior</p>
              <p className="text-xs text-navy-500 dark:text-zinc-500 font-mono break-all bg-navy-50 dark:bg-zinc-800 rounded p-2">{invoice.verifactu_prev_hash || "0"}</p>
            </div>
            <div className="md:col-span-2">
              <p className="text-xs text-navy-500 dark:text-zinc-400 mb-1">URL verificación QR</p>
              <p className="text-xs text-navy-700 dark:text-zinc-300 font-mono break-all bg-navy-50 dark:bg-zinc-800 rounded p-2">{invoice.verifactu_qr_data || "—"}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {fiscalTimeline.length > 0 && (
        <div className="bg-white dark:bg-zinc-900 border border-navy-100 dark:border-zinc-800 shadow-sm dark:shadow-none rounded-xl p-5 mb-6">
          <h3 className="text-sm font-semibold text-brand-green uppercase tracking-wider mb-4">Trazabilidad fiscal</h3>
              ...
              return (
                <div key={ev.id} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div className="w-8 h-8 rounded-full bg-navy-100 dark:bg-zinc-800 flex items-center justify-center text-sm flex-shrink-0">
                      {ev.icon}
                    </div>
                    {!isLast && <div className="w-0.5 flex-1 min-h-[16px] bg-navy-100 dark:bg-zinc-800" />}
                  </div>
                  <div className="pb-4 min-w-0">
                    <p className="text-sm font-medium text-navy-900 dark:text-white">{ev.label}</p>
                    ...
                    {ev.event_data && Object.keys(ev.event_data).length > 0 && ev.event_type === "hash_generated" && (
                      <p className="text-xs text-navy-500 dark:text-zinc-500 font-mono mt-0.5 truncate max-w-md">
      ...
      {invoice.payment_status !== "paid" && invoice.status !== "cancelled" && invoice.status !== "draft" && (
        <div className="bg-white dark:bg-zinc-900 border border-navy-100 dark:border-zinc-800 shadow-sm dark:shadow-none rounded-xl p-5 mb-6 print:hidden">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-brand-green uppercase tracking-wider">Cobros</h3>
            <button
              onClick={() => setShowPaymentForm(!showPaymentForm)}
              className="px-3 py-1.5 bg-brand-green text-white rounded-lg text-xs font-semibold hover:opacity-90 transition"
            >
          ...
          <div className="mb-4">
            <div className="flex justify-between text-xs mb-1">
              <span className="text-navy-500 dark:text-zinc-400">Cobrado</span>
              <span className="text-navy-700 dark:text-zinc-300 tabular-nums">
            ...
            <div className="h-2 w-full rounded-full bg-navy-100 dark:bg-zinc-800">
              <div
          ...
          {/* Payment form */}
          {showPaymentForm && (
            <div className="bg-navy-50 dark:bg-zinc-800 rounded-lg p-4 mb-4 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="text-xs text-navy-500 dark:text-zinc-400 block mb-1">Importe (€)</label>
                  <input
                    type="number"
                    step="0.01"
                    value={paymentAmount}
                    ...
                    className="w-full px-3 py-2 rounded-lg bg-white dark:bg-zinc-800 border border-navy-200 dark:border-zinc-700 text-navy-900 dark:text-white text-sm focus:outline-none focus:border-brand-green"
                  />
                </div>
                <div>
                  <label className="text-xs text-navy-500 dark:text-zinc-400 block mb-1">Método</label>
                  <select
                    value={paymentMethod}
                ...
                <div>
                  <label className="text-xs text-navy-500 dark:text-zinc-400 block mb-1">Referencia</label>
                  <input
                    type="text"
                    value={paymentRef}
              ...
              <button
                ...
                className="px-4 py-2 bg-brand-green text-white rounded-lg text-sm font-semibold hover:opacity-90 transition disabled:opacity-50"
              >
          ...
          {invoicePayments.length > 0 && (
            <div className="space-y-2">
              {invoicePayments.map((p) => (
                <div key={p.id} className="flex items-center justify-between py-2 border-b border-navy-100 dark:border-zinc-800 last:border-0">
                  <div>
                    <p className="text-sm text-navy-700 dark:text-zinc-200">
                      {paymentMethodLabels[p.payment_method] || p.payment_method}
                      {p.reference && <span className="ml-2 text-navy-500 dark:text-zinc-500 font-mono text-xs">{p.reference}</span>}
                    </p>
                    <p className="text-xs text-navy-500 dark:text-zinc-500">
      ...
      <div className="flex flex-wrap gap-3 print:hidden">
        <button onClick={handleDownloadXML}
          className="px-4 py-2 bg-white dark:bg-zinc-900 text-navy-700 dark:text-zinc-200 border border-navy-200 dark:border-zinc-800 rounded-lg text-sm font-medium hover:bg-navy-50 hover:border-navy-300 dark:hover:bg-zinc-800 dark:hover:border-zinc-700 transition">
        ...
        {invoice.status === "draft" && lines.length > 0 && (
          <button onClick={() => handleStatusChange("issued")}
            className="px-5 py-2 bg-brand-green text-white rounded-lg text-sm font-medium hover:opacity-90 transition">
            Emitir factura
          </button>
        )}
      </div>
      ...
      {invoice.notes && (
        <div className="bg-white dark:bg-zinc-900 border border-navy-100 dark:border-zinc-800 shadow-sm dark:shadow-none rounded-xl p-4 mt-6">
          <p className="text-xs text-navy-500 dark:text-zinc-400 mb-1">Notas</p>
          <p className="text-sm text-navy-700 dark:text-zinc-300">{invoice.notes}</p>
        </div>
      )}
    </div>
  );
