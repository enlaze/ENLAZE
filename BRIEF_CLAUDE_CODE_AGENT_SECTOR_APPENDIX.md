# Apéndice — Perfiles de inteligencia por sector (listos para pegar)

Contenido de referencia para `lib/agent/sector-intel.ts` (ver `BRIEF_CLAUDE_CODE_AGENT_SECTOR.md`).
Las claves coinciden con `sectorConfigs` de `lib/agent-prompts.ts`. Las `news_queries`
pueden contener el placeholder `{ciudad}`, que `/api/agent/config` debe interpolar con la
ciudad del negocio (`profiles.city`) antes de enviárselo a n8n / al endpoint de noticias.

`months` usa 1–12 (enero=1). Ajusta libremente; son contenido editorial, no contrato de API.
...
- En `/api/agent/config`: resuelve `getSectorIntel(business_sector)` y devuelve el perfil con `news_queries` ya pasadas por `resolveNewsQueries(profile, city)`.
- En `/api/agent/news`: usa esas queries para Google News RSS. Mapea cada item a la fila de `agent_news`.
- En `Build Claude Prompt` (n8n): pasa `kpis_focus`, `seasonal_focus`, `campaign_archetypes`, `regulatory_notes` al `ctx` y aplica las reglas de "ESPECIALIZACIÓN POR SECTOR" del brief principal.
- Si más adelante quieres distinguir, p.ej., "panadería" dentro de comercio, basta con añadir una clave nueva aquí y como opción de onboarding — la arquitectura ya lo soporta.
