...
exports.generateBudgetPDFHTML = generateBudgetPDFHTML;
...
function buildHeaderHTML(budget, isInternal) {
...
function buildClientInfoHTML(budget) {
    return `
...
function generateClientPDFHTML(budget, clientView, serviceLabelsMap) {
    ...
    const chaptersHTML = clientView.chapters.map(ch => {
        chapterIdx++;
        const tasksHTML = ch.includedTasks.map(t => `<li class="task-item">${t}</li>`).join("");
        return `
...
function generateInternalPDFHTML(budget, internalView, serviceLabelsMap) {
    const sLabel = (serviceLabelsMap && serviceLabelsMap[budget.service_type]) ||
        fallbackServiceLabels[budget.service_type] || budget.service_type;
    let chapterIdx = 0;
 ...
 */
function generateBudgetPDFHTML(budget, items, mode, serviceLabelsMap, categoryLabelsMap) {
