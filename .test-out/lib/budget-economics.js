...
exports.calculateEconomics = calculateEconomics;
exports.checkUndervaluation = checkUndervaluation;
exports.recalculateItem = recalculateItem;
exports.calculateRepriceImpact = calculateRepriceImpact;
...
function getMarketRange(projectType, quality) {
    const match = MARKET_RANGES.find((r) => r.project_type === projectType && r.quality === quality);
    if (match)
        return { min: match.min_eur_m2, max: match.max_eur_m2 };
 ...
 */
function calculateEconomics(items, preferences, projectType, surfaceM2, workersCount, calendarDays) {
    // ── Cost breakdown ──
    ...
    const undervaluationCheck = checkUndervaluation(items, costBreakdown, perM2, preferences, projectType, surfaceM2, workersCount ?? null, calendarDays ?? null);
    // ── Chapter breakdown ──
...
// ─── Cost breakdown ─────────────────────────────────────────────────────────
function calculateCostBreakdown(items, preferences) {
    let materials = 0;
    let labor = 0;
    let machinery = 0;
...
// ─── Per m2 analysis ────────────────────────────────────────────────────────
function calculatePerM2(costBreakdown, surfaceM2, projectType, quality) {
 ...
 */
function checkUndervaluation(items, costBreakdown, perM2, preferences, projectType, surfaceM2, workersCount, calendarDays) {
    const warnings = [];
...
function checkLaborHours(items) {
    let totalExpected = 0;
    let totalActual = 0;
    const deficientChapters = [];
    ...
    for (const [chapter, chapterItems] of byChapter) {
        ...
        if (chapterExpected > 0 && chapterActual < chapterExpected * 0.7) {
            deficientChapters.push(CHAPTER_LABELS[chapter] || chapter);
        }
    }
    if (totalExpected > 0 && totalActual < totalExpected * 0.7) {
        return {
            ...
            severity: totalActual < totalExpected * 0.5 ? "high" : "medium",
        };
    }
    return null;
}
function checkCheapMaterials(items) {
    ...
    for (const item of items) {
        if (!materialHeavyChapters.has(item.chapter))
            continue;
        if (item.priority === "opcional")
            continue;
...
// ─── Chapter breakdown ──────────────────────────────────────────────────────
function calculateChapterBreakdown(items, defaultMarginPercent) {
    const byChapter = new Map();
    for (const item of items) {
        const list = byChapter.get(item.chapter) || [];
        list.push(item);
...
// ─── Price confidence summary ───────────────────────────────────────────────
function calculatePriceConfidence(items) {
 ...
 */
function recalculateItem(item, overrides) {
 ...
 */
function calculateRepriceImpact(oldItems, newItems, marginPercent) {
    const changes = [];
