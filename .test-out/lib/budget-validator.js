 ...
 */
function validateBudget(items, economics, analysis, scope, timeline) {
    const checks = [];
    const warnings = [];
    const suggestions = [];
...
// ─── COMPLETENESS checks ────────────────────────────────────────────────────
function runCompletenessChecks(items, analysis, checks, warnings, suggestions) {
...
// ─── QUANTITIES checks ──────────────────────────────────────────────────────
function runQuantityChecks(items, scope, checks, warnings, suggestions) {
    ...
    const sanitarioItems = items.filter(i => i.chapter === "sanitarios" && i.unit === "ud" && i.priority !== "opcional");
...
// ─── PRICING checks ─────────────────────────────────────────────────────────
function runPricingChecks(items, checks, warnings) {
    // Check: no zero prices
    const zeroPriceItems = items.filter(i => i.unit_cost <= 0 && i.priority !== "opcional" && i.chapter !== "protecciones");
    checks.push({
        category: "pricing",
    ...
    const lowConfidenceItems = items.filter(i => i.price_source === "estimated" && i.confidence_score < 0.50);
...
function runProfitabilityChecks(items, economics, checks, warnings) {
    ...
    const negativeMarginChapters = economics.chapter_breakdown.filter(ch => ch.margin_percent < 0);
...
// ─── DURATION checks ────────────────────────────────────────────────────────
function runDurationChecks(items, timeline, checks, warnings) {
    if (!timeline)
        return;
...
// ─── CONSISTENCY checks ─────────────────────────────────────────────────────
function runConsistencyChecks(items, economics, checks, _warnings) {
    // Check: subtotals = quantity * unit_price
    const inconsistentSubtotals = [];
    for (const item of items) {
    ...
    const totalFromChapters = round2(economics.chapter_breakdown.reduce((sum, c) => sum + c.direct_cost, 0));
...
function collectEstimatedItems(items) {
    return items
...
// ─── Score calculation ──────────────────────────────────────────────────────
function calculateScore(checks, validationWarnings) {
    if (checks.length === 0)
        return 100;
    let score = 100;
