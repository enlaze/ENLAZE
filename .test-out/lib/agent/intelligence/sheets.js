...
exports.emptySheetsIntel = emptySheetsIntel;
exports.fetchSheetsIntel = fetchSheetsIntel;
const LOG = "[agent/intel/sheets]";
const MAX_ROWS = 500;
function emptySheetsIntel(status, error = null) {
    return {
        connected: status === "ok",
        status,
        error_message: error,
...
async function ghttp(url, token) {
...
function detectSchema(headerRow, dataRows) {
    const cols = [];
    ...
    for (let i = 0; i < headerRow.length; i++) {
        ...
        for (let r = 0; r < sampleSize; r++) {
            ...
            if (n !== null) {
                ...
                stringHits++;
                if (distinctStrings.size < 60)
                    distinctStrings.add(cell);
            }
        }
        ...
        if (total === 0)
            type = "string";
        else if (dateHits / Math.max(total, 1) > 0.7)
            type = "date";
        else if (numberHits / Math.max(total, 1) > 0.7)
            type = "number";
        else if (stringHits / Math.max(total, 1) > 0.7)
            type = "string";
        let role = "unknown";
        ...
        if (type === "date" || /\b(fecha|date|d[ií]a)\b/.test(lowerName)) {
            ...
            if (/(ingreso|ingresos|total|importe|revenue|venta|ventas|facturaci[oó]n|€)/i.test(lowerName))
                role = "revenue";
            else if (/(precio|price|pvp|coste|cost)/i.test(lowerName))
                role = "price";
            else if (/(unidades|cantidad|qty|quantity|stock|n|num)/i.test(lowerName) || (avg > 0 && avg < 50 && Number.isInteger(avg)))
                role = "quantity";
            else if (avg >= 5)
                role = "revenue";
            else
    ...
    let confidence = "low";
    if (hasDate && hasProduct && hasNumeric)
        confidence = "high";
    else if (hasDate && hasNumeric)
        confidence = "medium";
    else if (hasNumeric)
        confidence = "low";
...
function parseRows(cols, rows) {
    ...
    return rows.map((r) => {
        ...
        if (revenue === null && quantity !== null && price !== null)
...
function computeSales(parsed) {
    ...
    const prevMonthRows = parsed.filter((r) => r.date && r.date >= prevMonthStart && r.date < monthStart);
...
function computeTopProducts(parsed) {
    const now = new Date();
    const today = startOfDay(now);
    const weekStart = new Date(today);
    ...
    if (curMap.size === 0)
        return [];
    const products = Array.from(curMap.entries()).map(([name, cur]) => {
...
function buildAlerts(parsed, topProducts) {
    const alerts = [];
    if (topProducts) {
    ...
    for (const r of parsed) {
        if (!r.date || r.revenue === null)
            continue;
        if (r.date < trailingStart)
            continue;
...
async function fetchSheetsIntel(args) {
    const t0 = Date.now();
    ...
    if (!spreadsheetId) {
        const driveRes = await ghttp("https://www.googleapis.com/drive/v3/files?q=mimeType='application/vnd.google-apps.spreadsheet'&orderBy=modifiedTime+desc&pageSize=1&fields=files(id,name,modifiedTime)", accessToken);
        if (!driveRes.ok) {
            if (driveRes.status === 401 || driveRes.status === 403)
                return emptySheetsIntel("auth_expired", `Drive ${driveRes.status}`);
            if (driveRes.status === 429)
                return emptySheetsIntel("rate_limited", "Drive 429");
            return emptySheetsIntel("api_error", `Drive ${driveRes.status} ${driveRes.statusText}`);
        }
        const file = (driveRes.body.files || [])[0];
        if (!file)
            return emptySheetsIntel("no_sheets_found", "No spreadsheets found in user's Drive");
    ...
    const meta = await ghttp(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}?fields=properties.title,sheets.properties`, accessToken);
    if (!meta.ok) {
        if (meta.status === 401 || meta.status === 403)
            return emptySheetsIntel("auth_expired", `Sheets meta ${meta.status}`);
        if (meta.status === 429)
            return emptySheetsIntel("rate_limited", "Sheets 429");
        return emptySheetsIntel("api_error", `Sheets meta ${meta.status} ${meta.statusText}`);
    }
    if (!spreadsheetName)
        spreadsheetName = meta.body.properties?.title || "(sin nombre)";
    ...
    const valsRes = await ghttp(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${range}?valueRenderOption=UNFORMATTED_VALUE&dateTimeRenderOption=FORMATTED_STRING`, accessToken);
    if (!valsRes.ok) {
        if (valsRes.status === 401 || valsRes.status === 403)
            return emptySheetsIntel("auth_expired", `Sheets values ${valsRes.status}`);
        if (valsRes.status === 429)
            return emptySheetsIntel("rate_limited", "Sheets values 429");
    ...
    console.log(`${LOG} sheet=${spreadsheetId} rows=${dataRows.length} confidence=${confidence} alerts=${alerts.length} fallback=${isFallback} elapsed_ms=${Date.now() - t0}`);
    return {
        connected: true,
        status: "ok",
        error_message: null,
