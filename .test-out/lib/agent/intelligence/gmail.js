...
exports.GMAIL_WINDOW_MAX_MESSAGES = exports.GMAIL_WINDOW_DAYS = void 0;
exports.emptyGmailIntel = emptyGmailIntel;
exports.fetchGmailIntel = fetchGmailIntel;
const email_importance_1 = require("./email-importance");
const LOG = "[agent/intel/gmail]";
exports.GMAIL_WINDOW_DAYS = 7;
exports.GMAIL_WINDOW_MAX_MESSAGES = 50;
const DETAIL_FETCH_CAP = 30; // we only fully fetch this many messages
const DAYS_90_MS = 90 * 24 * 60 * 60 * 1000;
function emptyGmailIntel(status, error = null) {
    return {
        connected: status === "ok",
        status,
        error_message: error,
...
function buildSupplierTokens(hints) {
...
function categorize(from, subject, snippet, knownClients, supplierTokens) {
    const text = `${subject} ${snippet}`.toLowerCase();
    const domain = from.email.split("@")[1] || "";
    const haystack = `${from.name} ${domain} ${text}`.toLowerCase();
    if (from.email && knownClients.has(from.email))
        return "customer";
    if (SUPPLIER_DOMAINS.has(domain) || SUPPLIER_RE.test(text))
        return "supplier";
    if (supplierTokens.some((t) => haystack.includes(t)))
        return "supplier";
    if (INVOICE_RE.test(text))
        return "supplier";
    if (ORDER_RE.test(text))
        return "customer";
    if (NOREPLY_RE.test(from.email))
        return "internal";
    if (/\b(promo|descuento|oferta|newsletter|unsubscribe)\b/i.test(text))
        return "spam";
    if (INTERNAL_FREEMAIL.has(domain))
        return "unknown";
    return "unknown";
}
function priorityFor(args) {
    ...
    if (URGENT_RE.test(text))
        return "urgent";
    if (args.isRecurring && args.category === "customer" && args.hoursWaiting >= 24)
        return "high";
    if (args.category === "supplier" && args.hoursWaiting >= 48)
        return "high";
    if (args.category === "spam")
        return "low";
    return "normal";
}
/**
...
function importanceForHeuristic(a) {
    ...
    if (a.priority === "urgent")
        return { importance: "critical", reason: "Marcado como urgente" };
    ...
    if (a.isInvoice)
        return { importance: "important", reason: "Factura / pago detectado" };
    ...
    if (a.isMeeting)
        return { importance: "important", reason: "Solicitud de reunión / cita" };
    if (a.isKnownClient || a.category === "customer")
        return { importance: "important", reason: "Correo de cliente" };
    if (a.category === "spam" || noReply)
        return { importance: "noise", reason: "Notificación automática / promo" };
    if (a.category === "internal")
        return { importance: "noise", reason: "Notificación interna" };
    if (a.category === "supplier")
        return { importance: "normal", reason: "Proveedor, sin urgencia" };
    if (a.category === "unknown")
        return null; // defer to Haiku
    return { importance: "normal", reason: "Correo legítimo, sin urgencia" };
}
const IMPORTANCE_ORDER = { critical: 0, important: 1, normal: 2, noise: 3 };
const VALID_CATEGORIES = new Set(["customer", "supplier", "lead", "internal", "spam", "unknown"]);
...
async function fetchGmailIntel(accessToken, myEmail, opts = {}) {
    const t0 = Date.now();
    ...
    const unreadList = await gmailGet("https://gmail.googleapis.com/gmail/v1/users/me/messages?q=is:unread+in:inbox&maxResults=1", accessToken);
    if (!unreadList.ok) {
    ...
    const afterEpoch = Math.floor((Date.now() - exports.GMAIL_WINDOW_DAYS * 24 * 60 * 60 * 1000) / 1000);
    const windowList = await gmailGet(`https://gmail.googleapis.com/gmail/v1/users/me/messages?q=in:inbox+after:${afterEpoch}&maxResults=${exports.GMAIL_WINDOW_MAX_MESSAGES}`, accessToken);
    if (!windowList.ok) {
    ...
    const details = [];
    for (const m of ids) {
        const detail = await gmailGet(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${m.id}?format=metadata&metadataHeaders=Subject&metadataHeaders=From&metadataHeaders=To&metadataHeaders=Date`, accessToken);
        if (!detail.ok)
            continue;
        const d = detail.body;
        ...
        const hasAttachment = !!d.payload?.filename ||
    ...
    for (const d of details) {
        ...
        if (!cur.name && d.from.name)
            cur.name = d.from.name;
        senderHistogram.set(d.from.email, cur);
    }
    const classifiedAll = [];
    const haikuItems = [];
    const invoices_detected = [];
    const meeting_requests = [];
    const threads_count_by_category = {
        customer: 0,
        supplier: 0,
        lead: 0,
        internal: 0,
    ...
    for (const [threadId, msgs] of byThread.entries()) {
        ...
        const isRecurring = (senderStat?.count || 0) > 1 ||
    ...
    if (opts.enableHaiku && opts.sectorContext && haikuItems.length > 0) {
        const results = await (0, email_importance_1.classifyEmailsWithHaiku)(haikuItems, opts.sectorContext);
        if (results) {
    ...
    // 6) Tally + sort
    const importance_counts = { critical: 0, important: 0, normal: 0, noise: 0 };
    for (const t of classifiedAll)
        importance_counts[t.importance] += 1;
    const sortFn = (a, b) => {
    ...
    const top_senders_30d = Array.from(senderHistogram.entries())
        .map(([email, stat]) => ({
        email,
        name: stat.name,
        count: stat.count,
        ...
        .slice(0, 8);
    console.log(`${LOG} processed=${details.length} unread=${total_unread} classified=${classifiedAll.length} ` +
    ...
    return {
        ...
        fetched_range_days: exports.GMAIL_WINDOW_DAYS,
    };
}
