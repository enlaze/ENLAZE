...
exports.SECTOR_INTEL = void 0;
exports.getSectorIntel = getSectorIntel;
exports.resolveNewsQueries = resolveNewsQueries;
const hosteleria = {
 ...
 */
function getSectorIntel(sectorKey) {
    const key = (sectorKey || "").toLowerCase().trim();
    if (exports.SECTOR_INTEL[key])
        return exports.SECTOR_INTEL[key];
    if (key === "comercio_local" || key === "retail" || key === "comercio")
        return exports.SECTOR_INTEL.comercio;
    if (key === "peluqueria" || key === "belleza")
        return exports.SECTOR_INTEL.estetica;
    if (key === "restaurante" || key === "bar" || key === "cafeteria")
        return exports.SECTOR_INTEL.hosteleria;
    return exports.SECTOR_INTEL.default;
}
/**
 * Interpola {ciudad} (y futuros placeholders) en las queries de noticias.
 * Llamar en /api/agent/config con la ciudad del negocio.
 */
function resolveNewsQueries(profile, city) {
