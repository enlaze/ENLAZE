 ...
 */
function calculateTimeline(items, analysis, workersCount, startDate, deadlineDate) {
    ...
    const applicableDrying = DRYING_TIMES.filter(dt => {
        ...
        if (afterLower.includes("impermeabilizacion") && activeChapters.has("impermeabilizacion"))
            return true;
        if ((afterLower.includes("enfoscado") || afterLower.includes("albanileria")) && activeChapters.has("albanileria"))
            return true;
        if (afterLower.includes("nivelacion") && activeChapters.has("pavimentos"))
            return true;
        if (afterLower.includes("pintura") && activeChapters.has("pintura"))
            return true;
        if ((afterLower.includes("solado") || afterLower.includes("pavimento")) && activeChapters.has("pavimentos"))
            return true;
        return false;
    });
    // ── Risks ──
...
function calculateChapterHours(items) {
...
// ─── Crew recommendation ────────────────────────────────────────────────────
function recommendCrew(items, chapterHours, userWorkersCount) {
    // Aggregate hours by trade
    const tradeHours = new Map();
...
function buildPhases(items, chapterHours, crew) {
...
function topologicalSort(chapters) {
    const chaptersSet = new Set(chapters);
    const visited = new Set();
    const result = [];
    function visit(chapter) {
        if (visited.has(chapter))
            return;
        visited.add(chapter);
...
// ─── Critical path ──────────────────────────────────────────────────────────
function calculateCriticalPath(phases) {
    if (phases.length === 0)
        return [];
