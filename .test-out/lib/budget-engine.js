...
exports.buildScopeQuantities = buildScopeQuantities;
exports.normalizeBudgetItemsToScope = normalizeBudgetItemsToScope;
exports.calculateItemCostBreakdown = calculateItemCostBreakdown;
exports.buildScopeMaterials = buildScopeMaterials;
exports.getMarketRange = getMarketRange;
exports.adjustToMarket = adjustToMarket;
exports.estimateRealisticTimeline = estimateRealisticTimeline;
exports.inferClimaSystem = inferClimaSystem;
exports.buildChapterTechnicalBreakdown = buildChapterTechnicalBreakdown;
exports.buildClientView = buildClientView;
exports.buildInternalView = buildInternalView;
// ─── A. Scope Quantities ────────────────────────────────────────────────────
function buildScopeQuantities(scope) {
...
function detectChapter(concept, description) {
 ...
 */
function normalizeBudgetItemsToScope(scope, items, marginMultiplier) {
    ...
    const add = (chapter, concept, desc, qty, unit, price, cat) => {
    ...
    add("protecciones", "Proteccion de zonas comunes y ascensor", "Forrado de ascensor, pasillos y elementos comunes con carton ondulado y plastico.", 1, "PA", Math.max(q.floorArea * 1.5, 150), "otros");
    add("demoliciones", "Demoliciones y retirada", "Demolicion de revestimientos, tabiques y pavimentos existentes.", q.demolitionArea, "m2", 15, "mano_obra");
    add("albanileria", "Formacion de tabiqueria", "Suministro e instalacion de nueva tabiqueria interior (Pladur/Ladrillo).", q.partitionArea, "m2", 38, "mano_obra");
    add("falsos_techos", "Falsos techos continuos", "Formacion de falso techo de placa de yeso laminado.", q.ceilingArea, "m2", 28, "mano_obra");
    add("fontaneria", `Instalacion de fontaneria (${q.bathroomsCount} bano${q.bathroomsCount > 1 ? "s" : ""} + ${q.kitchenIncluded ? "cocina" : "sin cocina"})`, `Red de agua fria, caliente y desagues para ${q.bathroomsCount} bano(s)${q.kitchenIncluded ? " y cocina" : ""}.`, 1, "PA", (q.bathroomsCount * 950) + (q.kitchenIncluded ? 650 : 0), "mano_obra");
    add("electricidad", "Instalacion electrica completa", `Cuadro general, ${q.electricalPointsEstimated} puntos de luz/enchufes, mecanismos y protecciones.`, q.electricalPointsEstimated, "ud", 42, "mano_obra");
    add("impermeabilizacion", "Impermeabilizacion zonas humedas", "Lamina impermeabilizante en banos y cocina.", Math.round(q.wetWallArea * 0.4), "m2", 22, "material");
    add("revestimientos", "Alicatado de zonas humedas", "Colocacion de revestimiento ceramico en paredes de banos y cocina.", q.wetWallArea, "m2", 38, "mano_obra");
    add("pavimentos", "Solado general de vivienda", "Colocacion de pavimento ceramico o laminado.", q.pavementArea, "m2", 32, "mano_obra");
    add("rodapie", "Rodapie", "Suministro y colocacion de rodapie a juego.", q.baseboardMlEstimated, "ml", 9, "mano_obra");
    add("pintura", "Pintura plastica lisa (paredes y techos)", "Preparacion de superficie y aplicacion de dos manos de pintura plastica lavable.", q.wallPaintArea + q.ceilingArea, "m2", 9, "mano_obra");
    add("carpinteria_interior", "Carpinteria interior (puertas de paso)", "Suministro y colocacion de puertas de paso lacadas con herrajes.", q.doorsEstimated, "ud", 380, "material");
    if (scope.incluye_ventanas) {
        add("carpinteria_exterior", "Carpinteria exterior (ventanas)", "Suministro y colocacion de ventanas aluminio RPT con doble acristalamiento.", q.windowsCountEstimated, "ud", 650, "material");
    }
    add("sanitarios", `Sanitarios y griferia (${q.bathroomsCount} bano${q.bathroomsCount > 1 ? "s" : ""})`, `Suministro e instalacion de inodoro, lavabo, plato de ducha y griferia para ${q.bathroomsCount} bano(s).`, q.bathroomsCount, "lote", 1800, "material");
    if (scope.incluye_cocina) {
        add("cocina", "Cocina completa", "Muebles altos y bajos, encimera, fregadero y griferia de cocina.", 1, "PA", q.floorArea > 120 ? 7000 : q.floorArea > 80 ? 5500 : 4500, "material");
    }
    if (scope.incluye_climatizacion) {
        ...
        add("climatizacion", `Climatizacion — ${clima.label}`, clima.description, climaQty, climaUnit, climaPrice, "mano_obra");
    }
    add("residuos", "Gestion de residuos y contenedores", "Carga, transporte a vertedero autorizado y tasa.", q.wasteContainersEstimated, "ud", 290, "otros");
    add("seguridad", "Seguridad y medios auxiliares", "Protecciones colectivas, EPI, señalizacion y medios auxiliares durante obra.", 1, "PA", Math.max(q.floorArea * 6, 400), "otros");
    add("limpieza", "Limpieza final de obra", "Limpieza profesional exhaustiva de todas las estancias.", 1, "PA", Math.max(q.floorArea * 3.5, 300), "mano_obra");
    return [...corrected, ...missing];
}
// ─── C. Cost Breakdown ──────────────────────────────────────────────────────
function calculateItemCostBreakdown(item, _scope, marginPct) {
    const ch = item.chapter;
 ...
 */
function buildScopeMaterials(scope) {
    const q = buildScopeQuantities(scope);
...
// ─── E. Market Adjustment ───────────────────────────────────────────────────
function getMarketRange(scope, serviceType) {
 ...
 */
function adjustToMarket(scope, items, materials, serviceType, marginMultiplier) {
    // ─── GUARD A: SAFE AREA ──────────────────────────────────────────────
    ...
    const scaledItems = items.map(p => {
        ...
        const newCostBreakdown = p.cost_breakdown
            ? {
                ...p.cost_breakdown,
...
function estimateRealisticTimeline(scope, items) {
    ...
    if (scope.incluye_cocina)
        assumptions.push("Incluye cocina completa (mobiliario + instalaciones).");
    if (scope.incluye_ventanas)
        assumptions.push("Incluye cambio de ventanas (puede requerir licencia municipal).");
    if (scope.incluye_climatizacion)
        assumptions.push("Incluye climatizacion (coordinacion con instalador externo).");
 ...
 */
function inferClimaSystem(scope) {
    const area = scope.superficie_m2;
 ...
 */
function buildChapterTechnicalBreakdown(chapter, scope, q) {
 ...
 */
function buildClientView(scope, items, ivaPct) {
    const q = buildScopeQuantities(scope);
    ...
    for (const item of items) {
        ...
        if (!chapterGroups.has(ch))
            chapterGroups.set(ch, []);
 ...
 */
function buildInternalView(scope, items, materials, ivaPct, _resolvedPrices) {
    const climaSpec = scope.incluye_climatizacion ? inferClimaSystem(scope) : undefined;
    ...
    for (const mat of materials) {
        ...
        if (!materialsByChapter.has(ch))
            materialsByChapter.set(ch, []);
    ...
    for (const ch of chapterOrder) {
        ...
        const materialLines = chMaterials.map(mat => {
