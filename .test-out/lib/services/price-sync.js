...
exports.syncUserPrices = syncUserPrices;
async function syncUserPrices(supabase, userId, sector, marketPrices, syncSource = "manual") {
    ...
    try {
        ...
        for (const mp of marketPrices) {
            ...
            if (subcat === "Fontaneria")
                subcat = "Fontanería";
            if (subcat === "Albanileria")
                subcat = "Albañilería";
            ...
            if (meta.purchase_price !== undefined)
                updateData.purchase_price = parseFloat(meta.purchase_price);
            if (meta.recommended_sale_price !== undefined)
                updateData.recommended_sale_price = parseFloat(meta.recommended_sale_price);
            if (meta.gross_margin_pct !== undefined)
                updateData.gross_margin_pct = parseFloat(meta.gross_margin_pct);
            if (meta.supplier_name !== undefined)
                updateData.supplier_name = meta.supplier_name;
            if (existingMap.has(lowerName)) {
                ...
                if (error)
                    console.error("[PriceSync] Insert error:", itemName, error);
                else
                    added++;
            }
        }
