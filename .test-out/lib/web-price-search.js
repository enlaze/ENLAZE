...
async function searchViaSerpAPI(req) {
    ...
    try {
        ...
        for (const item of shoppingResults.slice(0, 8)) {
            ...
            if (!url)
                continue; // Must have real URL to count as web_search
            results.push({
...
async function searchViaRetailerAPIs(req) {
 ...
 */
async function searchViaN8nWebhook(req) {
 ...
 */
async function searchWebPricesBatch(requests, concurrency = 3) {
